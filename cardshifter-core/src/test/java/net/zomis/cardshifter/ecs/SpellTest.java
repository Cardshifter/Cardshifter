...
import net.zomis.cardshifter.ecs.usage.CyborgChroniclesEffects;
...
public class SpellTest extends GameTest {
	...
	private CyborgChroniclesGame mod;
	
	private ResourceRetriever scrap = ResourceRetriever.forResource(CyborgChroniclesResources.SCRAP);

	@Test
	public void preventOpponentAction() {
		assertNotNull(currentPlayer());
		Effects eff = new Effects();
		Entity spell = mod.createSpell(NO_NAME, hand.get(currentPlayer()), 0, 0, eff.addSystem(e -> new OpponentCannotUseSystem(Players.findOwnerFor(e), CyborgChroniclesGame.END_TURN_ACTION)));
		useAction(spell, CyborgChroniclesGame.USE_ACTION);
		assertTrue(spell.isRemoved());
		nextPhase();
		useFail(currentPlayer(), CyborgChroniclesGame.END_TURN_ACTION);
	}
	...
	public void frezeOpponentMinion() {
		...
		Entity spell = mod.createTargetSpell(NO_NAME, hand.get(currentPlayer()), 0, 0, eff.giveTarget(e -> new UntilEndOfOwnerTurnSystem(e, new EntityCannotUseSystem(e, CyborgChroniclesGame.ATTACK_ACTION))),
			new FilterComponent(new Filters().isCreatureOnBattlefield()));
		useActionWithTarget(spell, CyborgChroniclesGame.USE_ACTION, freezeTarget);
		assertTrue(spell.isRemoved());
		nextPhase();
		useFail(freezeTarget, CyborgChroniclesGame.ATTACK_ACTION);
		nextPhase();
		nextPhase();
		useActionWithTarget(freezeTarget, CyborgChroniclesGame.ATTACK_ACTION, getOpponent());
	}
	
	@Test
	public void scrapAll() {
		assertNotNull(currentPlayer());
		CyborgChroniclesEffects eff = new CyborgChroniclesEffects();
		scrap.resFor(currentPlayer()).set(2);
		scrap.resFor(getOpponent()).set(3);
		assertResource(currentPlayer(), CyborgChroniclesResources.SCRAP, 2);
		assertResource(getOpponent(), CyborgChroniclesResources.SCRAP, 3);
		...
		assertResource(currentPlayer(), CyborgChroniclesResources.SCRAP, 2 + 4);
		assertResource(getOpponent(), CyborgChroniclesResources.SCRAP, 3 + 3);
	}

	@Override
	protected void setupGame(ECSGame game) {
		mod = new CyborgChroniclesGame();
